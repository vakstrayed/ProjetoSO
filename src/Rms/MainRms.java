...
import PrioridadeEstatica_2.MainPrioridadeEstatica;
import Util.Arquivo;
...
public class MainRms {
	...
	private static MainRms instance = null;
	...
	private static MainRms getInstance() {
	...
	public void PrioridadeEstatica() {
		...
		Arquivo.getInstance().arquivoSaida("Rms");
		// Arquivo.getInstance().arquivoSaida("ThroughPut: " +
		// this.getPrioEstaticThroughPUT());
		...
		while (fimTblProntos == false) {
			...
			if (tblExecutarVazia) {
				...
				if (!tblProntos.isEmpty()) {
					...
					p = tblProntos.stream().min((p1, p2) -> Integer.compare(p1.getPeriodo(), p2.getPeriodo())).get();

					processo = new Processo(p.getID(), p.getTempoChegada(), p.getTempoComputacao(), p.getTemposIO(),
			...
			if (!tblExecutarVazia) { // fluxo seguinte
				...
				while (executandoProcesso) {
					...
					// executando...
					if ((processo.containsTempoIO(PC) == false) && (processo.getTempoComputacaoNoPeriodo() > 0)) {

						aux = processo.getTempoComputacaoNoPeriodo();
						aux--;
						processo.setTempoComputacaoNoPeriodo(aux); // decrementa
																	// tempo
						// de computação
					...
					} else if (processo.containsTempoIO(PC) && (processo.getTempoComputacaoNoPeriodo() > 0)) { // chamada
						// i.o
					...
					} else if (processo.getTempoComputacaoNoPeriodo() == 0) { // acabou
																				// tempo
																				// de
																				// computacao

						processo.setTempoComputacaoNoPeriodo(processo.getTempoComputacao());
						executandoProcesso = false; // faz sair do loop de
						// execução
						tblExecutarVazia = true; // faz solicitar novo processo
													// a fila de
